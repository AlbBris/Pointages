...
${formatDateFr(dateForDay(weekInfo.year, weekInfo.week, day)).slice(0, 5)}`), 'Total'];
  ...
  function makeIbatSummarySheet(context) {
    ...
    sorted.forEach((person, personIndex) => {
      ...
      fillRange(cells, row, row, 1, 11, subtotalStyle);
      ...
      cells.push(makeCell(row, 4, 'TOTAL PERSONNE', subtotalStyle));
      ...
      cells.push(makeFormulaCell(row, 10, `SUM(E${row}:I${row})`, personWeekTotal(week, person.id), subtotalStyle));
      cells.push(makeCell(row, 11, statusParts.join(' · '), statusParts.length ? STYLE.HOURS_ALERT : subtotalStyle));
      row += 1;
    ...
    return {
      ...
      columns: [
        { min: 1, width: 10 }, { min: 2, width: 25 }, { min: 3, width: 22 }, { min: 4, width: 36 },
        { min: 5, max: 9, width: 12 }, { min: 10, width: 14 }, { min: 11, width: 32 }
      ],
      rowHeights: { 1: 28, 3: 24, 5: 40 },
      freeze: { ySplit: 5, xSplit: 4, topLeftCell: 'E6', activePane: 'bottomRight' },
      autoFilter: `A5:K${Math.max(5, totalRow - 1)}`,
      showGridLines: false, orientation: 'landscape', paperSize: 9,
      printArea: `A1:K${totalRow}`, repeatRows: '1:5', tabColor: 'FFFFD800',
  ...
  function makeVentilationSheet(context) {
    ...
    const staticHeaders = ['S/I/P', 'Entreprise / agence', 'Zone', 'Nom prénom', 'Total semaine'];
