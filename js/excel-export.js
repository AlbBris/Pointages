...
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  function typeHours(week, people, interim, day = null) {
    const byId = peopleByIdMap(people);
    return (week.entries || []).filter(entry => {
      const person = byId.get(entry.personId);
      return person && isInterim(person) === interim && (day === null || Number(entry.day) === Number(day));
    }).reduce((sum, entry) => sum + Number(entry.hours || 0), 0);
  }

  function taskTypeTotal(week, taskId, people, interim) {
    const byId = peopleByIdMap(people);
    return (week.entries || []).filter(entry => entry.taskId === taskId && byId.has(entry.personId) && isInterim(byId.get(entry.personId)) === interim)
      .reduce((sum, entry) => sum + Number(entry.hours || 0), 0);
  }

  function dayPresentCount(week, people, day, interim = null) {
    return people.filter(person => (interim === null || isInterim(person) === interim) && personDayTotal(week, person.id, day) > 0).length;
  }
  ...
  function makeSummarySheet(context) {
    ...
    const gccHours = typeHours(week, people, false);
    const interimHours = typeHours(week, people, true);
    ...
    const kpis = [
      { start: 1, end: 3, label: 'HEURES TOTALES', value: total },
      { start: 4, end: 6, label: 'HEURES GCC', value: gccHours },
      { start: 7, end: 9, label: 'HEURES INTÉRIM', value: interimHours },
...
${formatDateFr(dateForDay(weekInfo.year, weekInfo.week, day)).slice(0, 5)}`), 'Total'];
    ...
    const dailyRows = [
      ['Heures GCC', ...DAY_NAMES.map((_, day) => typeHours(week, people, false, day)), gccHours],
      ['Heures intérim', ...DAY_NAMES.map((_, day) => typeHours(week, people, true, day)), interimHours],
    ...
    ['Tâche', 'GCC', 'Intérim', 'Total', '% semaine', 'Personnes'].forEach((value, index) => {
      const col = [1, 6, 7, 8, 9, 10][index];
      cells.push(makeCell(taskStart + 1, col, value, STYLE.TABLE_HEADER));
    });
    merges.push(`A${taskStart + 1}:E${taskStart + 1}`, `J${taskStart + 1}:L${taskStart + 1}`);
    tasks.map(task => ({
      task,
      gcc: taskTypeTotal(week, task.id, people, false),
      interim: taskTypeTotal(week, task.id, people, true),
      ...
      cells.push(makeCell(row, 6, item.gcc, STYLE.TABLE_NUMBER, 'number'));
      cells.push(makeCell(row, 7, item.interim, STYLE.TABLE_NUMBER, 'number'));
  ...
  function makeTaskSummarySheet(context) {
    const { project, weekInfo, week, people, tasks } = context;
    const cells = [];
    const merges = ['A1:K1', 'A3:B3', 'C3:F3', 'G3:H3', 'I3:K3'];
...
${formatDateFr(dateForDay(weekInfo.year, weekInfo.week, day)).slice(0, 5)}`), 'Total', 'GCC', 'Intérim', '% semaine', 'Effectif'];
    ...
    tasks.forEach((task, index) => {
      ...
      cells.push(makeCell(row, 8, taskTypeTotal(week, task.id, people, false), STYLE.TABLE_NUMBER, 'number'));
      cells.push(makeCell(row, 9, taskTypeTotal(week, task.id, people, true), STYLE.TABLE_NUMBER, 'number'));
      cells.push(makeCell(row, 10, weekTotal(week) ? total / weekTotal(week) : 0, STYLE.PERCENT, 'number'));
      cells.push(makeCell(row, 11, new Set((week.entries || []).filter(entry => entry.taskId === task.id).map(entry => entry.personId)).size, STYLE.TABLE_NUMBER, 'number'));
    });
    const totalRow = 6 + tasks.length;
    fillRange(cells, totalRow, totalRow, 1, 11, STYLE.TOTAL);
    ...
    for (let col = 2; col <= 9; col += 1) {
      const cached = col <= 6 ? (week.entries || []).filter(entry => Number(entry.day) === col - 2).reduce((sum, entry) => sum + Number(entry.hours || 0), 0)
        : col === 7 ? weekTotal(week) : col === 8 ? typeHours(week, people, false) : typeHours(week, people, true);
      cells.push(makeFormulaCell(totalRow, col, `SUM(${colLetter(col)}6:${colLetter(col)}${Math.max(6, totalRow - 1)})`, cached, STYLE.TOTAL));
    }
    cells.push(makeCell(totalRow, 10, 1, STYLE.PERCENT, 'number'));
    cells.push(makeCell(totalRow, 11, new Set((week.entries || []).map(entry => entry.personId)).size, STYLE.TOTAL, 'number'));
    return {
      name: 'Synthèse tâches', cells, merges,
      columns: [{ min: 1, width: 35 }, { min: 2, max: 6, width: 12 }, { min: 7, max: 11, width: 12 }],
      rowHeights: { 1: 28, 3: 24, 5: 38 },
      freeze: { ySplit: 5, topLeftCell: 'A6', activePane: 'bottomLeft' },
      autoFilter: `A5:K${Math.max(5, totalRow - 1)}`,
      showGridLines: false, orientation: 'landscape', paperSize: 9,
      printArea: `A1:K${totalRow}`, repeatRows: '1:5', tabColor: 'FFA9D18E',
  ...
  function makeIbatSummarySheet(context) {
    ...
    const cells = [];
    const merges = ['A1:L1', 'A2:L2', 'C4:E4', 'H4:I4', 'K4:L4'];
    cells.push(makeCell(1, 1, 'SAISIE IBAT — HEURES PAR PERSONNE ET PAR TÂCHE', STYLE.TITLE));
    cells.push(makeCell(2, 1, 'LECTURE SIMPLIFIÉE : UNE LIGNE = UNE TÂCHE POUR UN COMPAGNON', STYLE.SUBTITLE));
    cells.push(makeCell(4, 1, 'CHANTIER', STYLE.META_LABEL));
    cells.push(makeCell(4, 3, project.name || '', STYLE.META_VALUE));
    cells.push(makeCell(4, 6, 'SEMAINE', STYLE.META_LABEL));
    cells.push(makeCell(4, 8, `${weekInfo.week} / ${weekInfo.year}`, STYLE.META_VALUE));
    cells.push(makeCell(4, 10, 'PÉRIODE', STYLE.META_LABEL));
    cells.push(makeCell(4, 11, `${formatDateFr(dateForDay(weekInfo.year, weekInfo.week, 0))} au ${formatDateFr(dateForDay(weekInfo.year, weekInfo.week, 4))}`, STYLE.META_VALUE));

    const headers = ['Type', 'Nom prénom', 'Entreprise / agence', 'Tâche'];
    DAY_NAMES.forEach((name, day) => headers.push(`${name}\n${formatDateFr(dateForDay(weekInfo.year, weekInfo.week, day)).slice(0, 5)}`));
    headers.push('Total tâche', 'Statuts / retards', 'Observations');
    headers.forEach((header, index) => cells.push(makeCell(6, index + 1, header, STYLE.TABLE_HEADER)));

    const sorted = [...people].sort((a, b) => {
      const typeCmp = Number(isInterim(a)) - Number(isInterim(b));
      return typeCmp || String(a.name || '').localeCompare(String(b.name || ''), 'fr');
    });

    let row = 7;
    sorted.forEach(person => {
      const interim = isInterim(person);
      const textStyle = interim ? STYLE.BODY_TEXT_BLUE : STYLE.BODY_TEXT_GCC;
      const numberStyle = interim ? STYLE.BODY_NUMBER_BLUE : STYLE.BODY_NUMBER_GCC;
      ...
      taskIds
        ...
        .forEach(taskId => {
          ...
          cells.push(makeCell(row, 1, interim ? 'Intérim' : 'GCC', textStyle));
          cells.push(makeCell(row, 2, person.name || '', textStyle));
          cells.push(makeCell(row, 3, interim ? (person.company || '') : 'GCC', textStyle));
          cells.push(makeCell(row, 4, taskName, textStyle));
          DAY_NAMES.forEach((_, day) => {
            const value = taskId
              ...
              : 0;
            cells.push(makeCell(row, 5 + day, value || '', numberStyle, value ? 'number' : 'string'));
          });
          const cachedTaskTotal = taskId
            ? entries.filter(entry => entry.taskId === taskId).reduce((sum, entry) => sum + Number(entry.hours || 0), 0)
            : 0;
          cells.push(makeFormulaCell(row, 10, `SUM(E${row}:I${row})`, cachedTaskTotal, numberStyle));
          cells.push(makeCell(row, 11, '', textStyle));
          const taskNotes = taskId
            ? [...new Set(entries.filter(entry => entry.taskId === taskId).map(entry => String(entry.note || '').trim()).filter(Boolean))].join(' / ')
            : '';
          cells.push(makeCell(row, 12, taskNotes, STYLE.WRAP_TEXT));
          row += 1;
        });
      ...
      fillRange(cells, row, row, 1, 12, interim ? STYLE.SUBTOTAL_INTERIM : STYLE.SUBTOTAL_GCC);
      cells.push(makeCell(row, 1, interim ? 'Intérim' : 'GCC', interim ? STYLE.SUBTOTAL_INTERIM : STYLE.SUBTOTAL_GCC));
      cells.push(makeCell(row, 2, person.name || '', interim ? STYLE.SUBTOTAL_INTERIM : STYLE.SUBTOTAL_GCC));
      cells.push(makeCell(row, 3, interim ? (person.company || '') : 'GCC', interim ? STYLE.SUBTOTAL_INTERIM : STYLE.SUBTOTAL_GCC));
      cells.push(makeCell(row, 4, 'TOTAL PERSONNE', interim ? STYLE.SUBTOTAL_INTERIM : STYLE.SUBTOTAL_GCC));
      for (let col = 5; col <= 9; col += 1) {
        const day = col - 5;
        const cached = personDayTotal(week, person.id, day);
        cells.push(makeFormulaCell(row, col, `SUM(${colLetter(col)}${personStartRow}:${colLetter(col)}${personEndRow})`, cached, interim ? STYLE.SUBTOTAL_INTERIM : STYLE.SUBTOTAL_GCC));
      }
      cells.push(makeFormulaCell(row, 10, `SUM(E${row}:I${row})`, personWeekTotal(week, person.id), interim ? STYLE.SUBTOTAL_INTERIM : STYLE.SUBTOTAL_GCC));
      cells.push(makeCell(row, 11, statusParts.join(' · '), statusParts.length ? STYLE.HOURS_ALERT : (interim ? STYLE.SUBTOTAL_INTERIM : STYLE.SUBTOTAL_GCC)));
      cells.push(makeCell(row, 12, noteSummary(week, person.id), STYLE.WRAP_TEXT));
      row += 1;
    });
    ...
    cells.push(makeCell(totalRow, 10, weekTotal(week), STYLE.TOTAL_STRONG, 'number'));

    return {
      name: 'Saisie iBAT', cells, merges,
      columns: [
        { min: 1, width: 10 }, { min: 2, width: 28 }, { min: 3, width: 20 }, { min: 4, width: 34 },
        { min: 5, max: 9, width: 12 }, { min: 10, width: 13 }, { min: 11, width: 34 }, { min: 12, width: 38 }
      ],
      rowHeights: { 1: 28, 2: 22, 4: 24, 6: 40 },
      freeze: { ySplit: 6, xSplit: 4, topLeftCell: 'E7', activePane: 'bottomRight' },
      autoFilter: `A6:L${Math.max(6, totalRow - 1)}`,
      showGridLines: false, orientation: 'landscape', paperSize: 9,
      printArea: `A1:L${totalRow}`, repeatRows: '1:6', tabColor: 'FFFFD800',
  ...
  function makeIbatDetailSheet(context) {
    ...
    const tasksById = new Map(tasks.map(task => [task.id, task]));
    const cells = [];
    const headers = ['Date', 'Jour', 'Nom prénom', 'Type', 'Entreprise / agence', 'Tâche', 'Heures', 'Statut', 'Retard', 'Observation'];
    ...
    sorted.forEach(person => {
      DAY_NAMES.forEach((dayName, day) => {
        ...
        rows.forEach(([taskId, item]) => {
          ...
          cells.push(makeCell(row, 4, isInterim(person) ? 'Intérim' : 'GCC', STYLE.TABLE_TEXT));
          cells.push(makeCell(row, 5, isInterim(person) ? (person.company || '') : 'GCC', STYLE.TABLE_TEXT));
          cells.push(makeCell(row, 6, taskId ? (tasksById.get(taskId)?.name || 'Tâche supprimée') : '', STYLE.TABLE_TEXT));
          cells.push(makeCell(row, 7, item.hours || 0, STYLE.TABLE_NUMBER, 'number'));
          const status = absence ? absence.code : (dayEntries.length ? 'Présent' : 'Non renseigné');
          cells.push(makeCell(row, 8, status, absence ? STYLE.ABSENCE : (status === 'Non renseigné' ? STYLE.HOURS_ALERT : STYLE.TABLE_TEXT)));
          cells.push(makeCell(row, 9, late ? 'Oui' : '', late ? STYLE.HOURS_ALERT : STYLE.TABLE_TEXT));
          const notes = [...new Set([
            ...item.notes,
            absence && String(absence.note || '').trim() ? String(absence.note).trim() : '',
            late && String(late.note || '').trim() ? `Retard : ${String(late.note).trim()}` : ''
          ].filter(Boolean))].join(' / ');
          cells.push(makeCell(row, 10, notes, STYLE.WRAP_TEXT));
          row += 1;
        });
      });
    });
    return {
      ...
      columns: [
        ...
        { min: 5, width: 20 }, { min: 6, width: 34 }, { min: 7, width: 10 }, { min: 8, width: 15 },
        { min: 9, width: 10 }, { min: 10, width: 38 }
      ],
      freeze: { ySplit: 1, xSplit: 3, topLeftCell: 'D2', activePane: 'bottomRight' },
      autoFilter: `A1:J${Math.max(1, row - 1)}`,
      showGridLines: false, orientation: 'landscape', paperSize: 9,
  ...
  function makeVentilationSheet(context) {
    ...
    const staticHeaders = ['S/I', 'Agence / GCC', 'Zone', 'Nom prénom', 'Total semaine'];
...
${date}`, dayStyle));
    ...
    people.forEach((person, personIndex) => {
      ...
      const textStyle = interim ? STYLE.BODY_TEXT_BLUE : STYLE.BODY_TEXT_GCC;
      const numberStyle = interim ? STYLE.BODY_NUMBER_BLUE : STYLE.BODY_NUMBER_GCC;
      cells.push(makeCell(row, 1, interim ? 'I' : 'S', textStyle));
      cells.push(makeCell(row, 2, interim ? (person.company || '') : 'GCC', textStyle));
      cells.push(makeCell(row, 3, person.zone || project.defaultZone || '', textStyle));
    ...
    const totalRow = gccRow + 2;
    const taskTotalRow = gccRow + 3;
    const subtotalDefinitions = [
      { row: gccRow, label: 'SOUS-TOTAL GCC', code: 'S', style: STYLE.SUBTOTAL_GCC, cached: typeHours(week, people, false) },
      { row: interimRow, label: 'SOUS-TOTAL INTÉRIM', code: 'I', style: STYLE.SUBTOTAL_INTERIM, cached: typeHours(week, people, true) }
    ];
    subtotalDefinitions.forEach(def => {
      ...
      tasks.forEach((task, taskIndex) => DAY_NAMES.forEach((_, day) => {
        ...
        const cached = (week.entries || []).filter(entry => entry.taskId === task.id && Number(entry.day) === day && isInterim(people.find(person => person.id === entry.personId)) === (def.code === 'I'))
          .reduce((sum, entry) => sum + Number(entry.hours || 0), 0);
    ...
    cells.push(makeFormulaCell(totalRow, 5, `SUM(E${gccRow}:E${interimRow})`, weekTotal(week), STYLE.TOTAL_STRONG));
    tasks.forEach((task, taskIndex) => DAY_NAMES.forEach((_, day) => {
      const col = 6 + taskIndex * 5 + day;
      cells.push(makeFormulaCell(totalRow, col, `SUM(${colLetter(col)}${gccRow}:${colLetter(col)}${interimRow})`, taskTotal(week, task.id, day), STYLE.TOTAL_STRONG));
    }));
  ...
  function makePointageSummarySheet(context, type) {
    ...
    const filtered = people.filter(person => type === 'GCC' ? !isInterim(person) : isInterim(person));
    const cells = [];
    const merges = ['A1:J1', 'A2:J2', 'A3:B3', 'C3:F3', 'G3:H3', 'I3:J3', 'A4:B4', 'E4:F4', 'H4:I4'];
    cells.push(makeCell(1, 1, `FEUILLE DE POINTAGE — ${type === 'GCC' ? 'SALARIÉS GCC' : 'INTÉRIMAIRES'}`, STYLE.TITLE));
...
${formatDateFr(dateForDay(weekInfo.year, weekInfo.week, day)).slice(0, 5)}`), 'Total', 'Observations'];
    ...
    return {
      name: type === 'GCC' ? 'Pointages GCC' : 'Pointages Intérim', cells, merges,
...
${formatDateFr(dateForDay(weekInfo.year, weekInfo.week, day)).slice(0, 5)}`), 'Total', 'Théorique', 'Écart'];
  ...
  function makeIndividualInterimSheet(context, person, index) {
    ...
    cells.push(makeCell(9, 5, 'ZONE', STYLE.META_LABEL));
    cells.push(makeCell(9, 7, person.zone || project.defaultZone || '', STYLE.META_VALUE));
  ...
  function makeDetailsSheet(context) {
    ...
    const tasksById = new Map(tasks.map(task => [task.id, task]));
    const cells = [];
    const headers = ['Date', 'Jour', 'Nom prénom', 'Type', 'Entreprise', 'Zone', 'Tâche', 'Heures', 'Observation'];
    headers.forEach((header, index) => cells.push(makeCell(1, index + 1, header, STYLE.TABLE_HEADER)));
    (week.entries || []).slice().sort((a, b) => Number(a.day) - Number(b.day) || String(peopleById.get(a.personId)?.name || '').localeCompare(String(peopleById.get(b.personId)?.name || ''), 'fr')).forEach((entry, index) => {
      ...
      cells.push(makeCell(row, 6, person?.zone || project.defaultZone || '', STYLE.TABLE_TEXT));
