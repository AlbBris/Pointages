(() => {
  ...
  function normalizeProject(project) {
    ...
    delete normalized.project.defaultZone;
    normalized.people ||= [];
    normalized.people = normalized.people.map(person => {
      ...
      return { ...person, type, zone: '', travelZone: String(person?.travelZone || '') };
    });
    normalized.tasks ||= [];
    normalized.teams ||= [];
  ...
  function pointageFilteredPeople({ applyMobileStatus = true } = {}) {
    ...
    return activePeople().filter(person => {
      ...
      if (search && ![person.name, person.company, person.travelZone, ...teamForPerson(person.id).map(team => team.name)].some(value => String(value || '').toLocaleLowerCase('fr').includes(search))) return false;
        ...
        <div class="person-card-main">
          ...
          <div class="person-identity">
            ...
            <div class="person-meta">
              ...
              ${person.travelZone ? `<span>• ${escapeHtml(person.travelZone)}</span>` : ''}
  ...
  function renderWeek() {
    ...
    document.getElementById('week-table-body').innerHTML = people.map(person => {
      ...
      return `<tr><td><div class="person-cell"><strong>${escapeHtml(person.name)}</strong><small><span class="badge ${typeClass}">${escapeHtml(person.type)}</span> ${escapeHtml(person.company || '')}${person.travelZone ? ` · ${escapeHtml(person.travelZone)}` : ''}${teamNames ? ` · ${escapeHtml(teamNames)}` : ''}</small></div></td>${cells}<td><strong>${numberText(personWeekTotal(person.id))}</strong></td></tr>`;
    }).join('');
  ...
  function filteredPersonnel() {
    ...
    return db.people.filter(person => {
      ...
      return [person.name, person.company, person.travelZone, person.type].some(value => String(value || '').toLocaleLowerCase('fr').includes(search));
    });
  }
  ...
  function renderPersonnel() {
    ...
    list.innerHTML = people.length ? people.map(person => {
      ...
      <div class="management-row personnel-management-row ${person.active === false ? 'inactive' : ''} ${ui.selectedPersonnel.has(person.id) ? 'selected-row' : ''}">
        ...
        <div><strong>${escapeHtml(person.travelZone || '—')}</strong><small>${teams.length ? escapeHtml(teams.map(team => team.name).join(', ')) : 'Aucune équipe'}</small></div>
  ...
  function renderRosterDialogList() {
    ...
    const rosterIds = new Set(currentWeek().rosterIds || []);
    const people = db.people.filter(person => person.active !== false || rosterIds.has(person.id)).filter(person => !query || [person.name, person.company, person.travelZone].some(value => String(value || '').toLocaleLowerCase('fr').includes(query)));
    document.getElementById('roster-list').innerHTML = people.length ? people.map(person => `<label class="roster-person ${person.active === false ? 'inactive' : ''}"><input type="checkbox" value="${escapeHtml(person.id)}" ${rosterIds.has(person.id) ? 'checked' : ''}><span><strong>${escapeHtml(person.name)}</strong><small>${escapeHtml(person.type)} · ${escapeHtml(person.company || '')}${person.travelZone ? ` · ${escapeHtml(person.travelZone)}` : ''}${person.active === false ? ' · inactive' : ''}</small></span></label>`).join('') : '<div class="empty-state"><strong>Aucune personne trouvée</strong></div>';
  }
...
Cordialement`;
  ...
  function openPointageDialog(personId, day) {
    ...
    document.getElementById('pointage-dialog-meta').textContent = `${person.type} · ${person.company || 'Sans entreprise'}${person.travelZone ? ` · ${person.travelZone}` : ''}${isWeekLocked() ? ' · lecture seule' : ''}`;
  ...
  function openPersonDialog(personId = null, presetType = '') {
    ...
    document.getElementById('person-travel-zone').value = person?.travelZone || '';
    const travelZoneField = document.getElementById('person-travel-zone-field');
    if (travelZoneField) travelZoneField.hidden = initialType === 'GCC';
  ...
  function savePerson(event) {
    ...
    const data = { id, name, type, company, zone: '', travelZone: type === 'GCC' ? '' : document.getElementById('person-travel-zone').value.trim().toUpperCase(), active: document.getElementById('person-active').checked, schedule };
  ...
  function exportCsv() {
    const rows = [['Chantier', 'Code', 'Semaine', 'Date', 'Jour', 'Nom', 'Type', 'Entreprise', 'Zone de déplacement', 'Tâche', 'Heures', 'Observation', 'Statut']];
    currentWeek().entries.forEach(entry => {
      ...
      rows.push([db.project.name, db.project.code, db.currentWeek.week, dateIso(entry.day), DAY_NAMES[entry.day], person?.name || '', person?.type || '', person?.company || '', person?.travelZone || '', task?.name || '', Number(entry.hours || 0), observation, late ? 'Retard' : 'Présent']);
    });
    currentWeek().absences.forEach(item => {
      const person = getPerson(item.personId);
      rows.push([db.project.name, db.project.code, db.currentWeek.week, dateIso(item.day), DAY_NAMES[item.day], person?.name || '', person?.type || '', person?.company || '', person?.travelZone || '', '', '', item.note || '', item.code]);
    });
  ...
  function attachEvents() {
    ...
    document.getElementById('person-type').addEventListener('change', event => {
      const travelZoneField = document.getElementById('person-travel-zone-field');
      if (travelZoneField) travelZoneField.hidden = event.target.value === 'GCC';
      if (event.target.value === 'GCC') document.getElementById('person-travel-zone').value = '';
