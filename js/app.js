(() => {
  ...
  function createProjectData(meta = {}) {
    ...
    project.defaultSchedules = { GCC: [8, 8, 8, 8, 7], Interim: [7.5, 7.5, 7.5, 7.5, 7] };
    project.weeks = {};
    return project;
  }
  ...
  function normalizeProject(project) {
    ...
    normalized.defaultSchedules ||= { GCC: [8, 8, 8, 8, 7], Interim: [7.5, 7.5, 7.5, 7.5, 7] };
    ...
    try {
      ...
      if (saved) {
        ...
        if (parsed && Array.isArray(parsed.projects)) {
          return { version: 1.13, projects: parsed.projects.filter(isProjectDatabase).map(normalizeProject) };
        }
      }
      for (const key of LEGACY_STORAGE_KEYS) {
        ...
        if (hasMeaningfulLegacyData(legacy)) {
          ...
          const migratedStore = { version: 1.13, projects: [migrated] };
  ...
  function isPersonEligibleForWeek(person, year, weekNumber) {
    if (!person || person.type === 'GCC' || !db?.contractRules?.excludeExpiredFromNewWeeks) return true;
  ...
  function pointageContractCheck(personId, day) {
    ...
    if (!person || person.type === 'GCC') return { ok: true };
  ...
  function weekTotals() {
    ...
    const gcc = entries.filter(e => gccIds.has(e.personId)).reduce((sum, e) => sum + Number(e.hours || 0), 0);
    return { total, gcc, interim: total - gcc };
  }
  ...
  function personCard(person) {
    ...
    const typeClass = person.type === 'GCC' ? 'gcc' : 'interim';
  ...
  function renderRosterSummary() {
    ...
    document.getElementById('roster-summary').innerHTML = rosterPeople.length ? `<div class="roster-stat"><strong>${rosterPeople.length}</strong><span>présents</span></div><div class="roster-stat"><strong>${gcc}</strong><span>GCC</span></div><div class="roster-stat"><strong>${interim}</strong><span>intérim</span></div><div class="roster-stat"><strong>${hours(target)}</strong><span>cible semaine</span></div>` : '<div class="empty-state small-empty"><strong>Aucun présent sélectionné</strong>Choisissez l’équipe de la semaine avant le pointage.</div>';
  }
...
Cordialement`;
  ...
  function openPersonDialog(personId = null, presetType = '') {
    ...
    const defaultKey = initialType === 'GCC' ? 'GCC' : 'Interim';
  ...
  function renderDefaultScheduleInputs() {
    const schedules = db.defaultSchedules || { GCC: [8, 8, 8, 8, 7], Interim: [7.5, 7.5, 7.5, 7.5, 7] };
  ...
  function applyDefaultSchedule(type) {
    const personType = type === 'GCC' ? 'GCC' : 'Intérim';
    ...
    document.getElementById('person-type').addEventListener('change', event => {
      if (!document.getElementById('person-id').value) {
        if (event.target.value === 'GCC') {
          ...
          [...document.querySelectorAll('.schedule-input')].forEach((input, index) => input.value = (db.defaultSchedules?.Interim || [7.5, 7.5, 7.5, 7.5, 7])[index]);
        }
      }
    });
